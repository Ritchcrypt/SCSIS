...
Write-Host ""
Write-Host "TabangNow Android Development Launcher" -ForegroundColor Cyan
...
# Do not remove first. `adb reverse --remove` returns an error when no
# listener exists, which Windows PowerShell can promote to a terminating
# NativeCommandError under ErrorActionPreference=Stop.
#
# Plain `adb reverse` creates the rule and rebinds the listener when needed.
...
Write-Host "[5/5] Launching TabangNow..." -ForegroundColor Yellow
Write-Host ""
Write-Host "API_BASE_URL=http://127.0.0.1:$Port" -ForegroundColor DarkGray
Write-Host ""

flutter run "--dart-define=API_BASE_URL=http://127.0.0.1:$Port"
