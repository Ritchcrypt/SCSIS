@php
    $feed = $feed ?? app(\App\Services\WeatherDisasterFeedService::class)->feed();
...
<div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <div class="flex items-center gap-3">
                <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-blue-50 text-2xl">
                    {{ $weather['icon'] ?? '🌤️' }}
                </div>
    ...
    <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <div class="rounded-2xl bg-slate-50 p-4">
            ...
            <p class="mt-1 text-xs text-slate-500">
                Source: {{ $weather['source'] ?? 'Weather feed' }}
            </p>
        </div>
        ...
        @if ($advisories->count())
            <div class="mt-4 space-y-3">
                @foreach ($advisories as $advisory)
                    ...
                    <div class="rounded-2xl border p-4 {{ $advisoryClass }}">
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <span class="inline-flex w-fit rounded-full bg-white/70 px-3 py-1 text-xs font-black uppercase">
                                {{ ucfirst($severity) }}
                            </span>
                        </div>
                    </div>
